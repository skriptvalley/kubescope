...
- Last work: v1.0.0 — first stable release: tag + GitHub release + multi-arch image publish (headlined by the FB-11 Dusk UI redesign) [feedback]
...
- Next expected: FB-10 (hi) — make the GHCR package public + link it to the repo (now gates anonymous pulls of **both** `:0.1.0` and `:1.0.0`), then pull-smoke the published image via the canonical `docker run`; then FB-5/FB-4 (both lo) or new feedback.
