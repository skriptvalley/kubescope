...
- Next expected: FB-10 (hi) — make the GHCR package public + link it to the repo, then pull-smoke the published image via the canonical `docker run`; then FB-5/FB-4 (both lo) or new feedback.
- ADRs touched this session: **0009 new** — adopt the skriptvalley "Dusk" design system (OKLCH tokens, vendored fonts, theme toggle; records the metrics-server-via-dynamic-client data source for Story F).
