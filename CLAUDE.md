...
Go module: `github.com/skriptvalley/kubescope`. Image: `ghcr.io/skriptvalley/kubescope:latest`. Env vars are prefixed `KUBESCOPE_` — canonical set only: `KUBESCOPE_LISTEN_ADDR`, `KUBESCOPE_PORT`, `KUBESCOPE_KUBECONFIG` (since FB-8/ADR-0008 a colon-separated list of files and/or directories, kubectl merge precedence), `KUBESCOPE_READ_ONLY`, `KUBESCOPE_AUTH_MODE`, the basic-auth credential pair `KUBESCOPE_AUTH_BASIC_USERNAME` / `KUBESCOPE_AUTH_BASIC_PASSWORD` (added Sprint 8, recorded in ADR-0005; consulted only when `KUBESCOPE_AUTH_MODE=basic`), and `KUBESCOPE_ALLOW_KUBECONFIG_SET` (added FB-6/ADR-0007; since FB-8/ADR-0008 gates the kubeconfig source-registry mutation endpoints; default `false`). Do not invent new env vars beyond this set.
