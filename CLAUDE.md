...
| Frontend | React 18, TypeScript 5, Vite 5, TailwindCSS + shadcn/ui **re-themed to the skriptvalley "Dusk" design system** (raw-OKLCH tokens, light/system/dark toggle; ADR-0009), TanStack Query v5, TanStack Table v8, react-router, CodeMirror 6 (YAML, Sprint 5), xterm.js 5 (terminal, Sprint 6), **Cytoscape.js + cytoscape-fcose** (resource graph, FB-14/ADR-0011 — compound nodes; the view is `React.lazy`-split so its ~180 KB gzipped stays off first paint). Fonts self-hosted (no CDN, ADR-0002): `@fontsource/space-grotesk` + `@fontsource-variable/geist` + `@fontsource-variable/geist-mono` |
...
Go module: `github.com/skriptvalley/kubescope`. Image: `ghcr.io/skriptvalley/kubescope:latest`. Env vars are prefixed `KUBESCOPE_` — canonical set only: `KUBESCOPE_LISTEN_ADDR`, `KUBESCOPE_PORT`, `KUBESCOPE_KUBECONFIG` (since FB-8/ADR-0008 a colon-separated list of files and/or directories, kubectl merge precedence), `KUBESCOPE_READ_ONLY`, `KUBESCOPE_AUTH_MODE` (`none`|`basic`|`session`; `session` = sign-in page + expiring cookie, FB-21/ADR-0013), the operator credential pair `KUBESCOPE_AUTH_BASIC_USERNAME` / `KUBESCOPE_AUTH_BASIC_PASSWORD` (added Sprint 8, recorded in ADR-0005; consulted when `KUBESCOPE_AUTH_MODE` is `basic` — both required — or `session` — password required, username optional), `KUBESCOPE_AUTH_SESSION_KEY` (session mode only, optional random token secret, FB-21/ADR-0013), `KUBESCOPE_ALLOW_KUBECONFIG_SET` (added FB-6/ADR-0007; since FB-8/ADR-0008 gates the kubeconfig source-registry mutation endpoints; default `false`), and `KUBESCOPE_BASE_PATH` (added FB-19/ADR-0012; the reverse-proxy sub-path, e.g. `/kubescope`; default unset = root — the served shell carries a matching `<base href>`, so every SPA URL must go through `withBase()` in `web/src/lib/base.ts`). Do not invent new env vars beyond this set.
...
│   ├── graph/              # resource relationship graph builder (ADR-0011)
