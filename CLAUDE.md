...
Go module: `github.com/skriptvalley/kubescope`. Image: `ghcr.io/skriptvalley/kubescope:latest`. Env vars are prefixed `KUBESCOPE_` — canonical set only: `KUBESCOPE_LISTEN_ADDR`, `KUBESCOPE_PORT`, `KUBESCOPE_KUBECONFIG` (since FB-8/ADR-0008 a colon-separated list of files and/or directories, kubectl merge precedence), `KUBESCOPE_READ_ONLY`, `KUBESCOPE_AUTH_MODE`, the basic-auth credential pair `KUBESCOPE_AUTH_BASIC_USERNAME` / `KUBESCOPE_AUTH_BASIC_PASSWORD` (added Sprint 8, recorded in ADR-0005; consulted only when `KUBESCOPE_AUTH_MODE=basic`), `KUBESCOPE_ALLOW_KUBECONFIG_SET` (added FB-6/ADR-0007; since FB-8/ADR-0008 gates the kubeconfig source-registry mutation endpoints; default `false`), and `KUBESCOPE_BASE_PATH` (added FB-19/ADR-0012; the reverse-proxy sub-path, e.g. `/kubescope`; default unset = root — the served shell carries a matching `<base href>`, so every SPA URL must go through `withBase()` in `web/src/lib/base.ts`). Do not invent new env vars beyond this set.
