...
- **SPA** (`web/src/lib/base.ts`): `basePath` is read once from the `<base href>`. The router gets it as `basename`, and `request()` (fetch), `openStream()` (EventSource) and the exec WebSocket URL go through `withBase()`. With no `<base>` (Vite dev server, tests) everything stays at the root.
